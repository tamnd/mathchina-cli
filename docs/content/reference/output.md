...
describes what users get. Pick a format with `-o`, or let cmo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cmo <command> -o table   # aligned columns for reading
cmo <command> -o jsonl   # one JSON object per line, for piping
cmo <command> -o json    # a single JSON array
cmo <command> -o csv     # spreadsheet friendly
cmo <command> -o tsv     # tab-separated
cmo <command> -o url     # just the URL column
cmo <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cmo <command> --fields id,title,url
```
...
```bash
cmo <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cmo <command>            # a table, because this is a terminal
cmo <command> | wc -l    # JSONL, because this is a pipe
```
